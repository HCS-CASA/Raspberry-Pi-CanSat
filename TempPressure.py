...
if __name__ == '__main__':
	...
	with open('TempPressure.csv', 'w') as csvfile: #open csv file
		...
		csvwriter.writerow(list(myTempPressure.returnConstants().keys())) #print the constant names
		csvwriter.writerow(list(myTempPressure.returnConstants().values())) #print constant values - yes this is inificient code
		csvwriter.writerow(['Temp', 'Pressure']) #print the measurment names
		for i in range(0, 100):
			myTempPressure.getResults()
			csvwriter.writerow(myTempPressure.returnResults())
			time.sleep(1)
			print(i)